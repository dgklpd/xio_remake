  ...
  <view class="battle-page" :style="hudStyleVars">
    ...
    <view v-if="!gameStarted" class="start-overlay">
      ...
      <view class="start-modal">
        <text class="start-modal__title">{{ startModalTitle }}</text>
        <text class="start-modal__desc">{{ startModalDesc }}</text>

        <view v-if="onlineMode" class="start-modal__info">
          <view class="start-modal__player-row">
            <view class="start-modal__player-copy">
              <text class="start-modal__player-label">房主</text>
              <text class="start-modal__player-name">{{ onlineRoomState?.host.display_name || displayName }}</text>
            </view>
            <text
              class="start-modal__state-pill"
              :class="{
                'start-modal__state-pill--ready': onlineRoomState?.host.ready,
                'start-modal__state-pill--idle': !onlineRoomState?.host.ready,
              }"
            >
              {{ onlineRoomState?.host.ready ? "准备" : "未准备" }}
            </text>
          </view>

          <view class="start-modal__player-row">
            <view class="start-modal__player-copy">
              <text class="start-modal__player-label">客位</text>
              <text class="start-modal__player-name">{{ onlineGuestLine }}</text>
            </view>
            <text
              class="start-modal__state-pill"
              :class="{
                'start-modal__state-pill--ready': onlineRoomState?.guest?.ready,
                'start-modal__state-pill--idle': !onlineRoomState?.guest?.ready,
              }"
            >
              {{ onlineRoomState?.guest ? (onlineRoomState?.guest?.ready ? "准备" : "未准备") : "未加入" }}
            </text>
          </view>

          <view class="start-modal__room-line">
            <text class="start-modal__room-code">房间号 {{ onlineRoomCode || "--" }}</text>
            <view class="start-modal__copy-btn" @tap.stop="handleCopyRoomCode">
              <text class="start-modal__copy-icon">⧉</text>
            </view>
          </view>
        </view>

        <view class="start-modal__actions">
          <view
            class="start-modal__btn start-modal__btn--primary"
            :class="{ 'start-modal__btn--disabled': startPrimaryDisabled }"
            @tap="handleStartPrimaryAction"
          >
            <text>{{ startPrimaryLabel }}</text>
          </view>
          <view class="start-modal__btn start-modal__btn--ghost" @tap="handleGameExit">
            <text>{{ startSecondaryLabel }}</text>
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="battle-shell battle-shell--arena">
      <view class="battle-overlay-controls hud-panel" :style="overlayControlsStyle">
        <view class="battle-overlay-controls__score">
          <text>{{ opponentDisplayName }} {{ enemyScore }}</text>
        ...
        >
          <view class="player-hud__head">
            <view class="player-hud__identity">
              <view class="player-hud__avatar player-hud__avatar--enemy"><text>{{ opponentAvatarFallback }}</text></view>
              <view>
                <text class="player-hud__name">{{ opponentDisplayName }}</text>
              </view>
            </view>
...
import { buildOnlineRoomSocketUrl, clearSavedOnlineSession, getSavedOnlineSession, leaveOnlineRoom } from "@/utils/onlineApi";
...
const onlineSeat = ref<"host" | "guest">("host");
const onlineSessionToken = ref("");
...
function loadOnlineSession(options?: Record<string, string>) {
  const session = getSavedOnlineSession<OnlineSessionCache>();
  const routeRoomCode = options?.roomCode ?? "";
  const mode = options?.mode ?? "";
  if (mode !== "online") return false;
  if (!session || typeof session !== "object") return false;

  onlineMode.value = true;
  onlineRoomCode.value = routeRoomCode || session.room_code || "";
  onlineSeat.value = session.seat;
  onlineSessionToken.value = session.session_token;
  if (!profile.nickname && session.identity?.display_name) {
    profile.nickname = session.identity.display_name;
  }
  return Boolean(onlineRoomCode.value && onlineSessionToken.value);
}

function connectOnlineRoomBattle() {
  if (!onlineRoomCode.value || !onlineSessionToken.value) return;
  const socketUrl = buildOnlineRoomSocketUrl(onlineRoomCode.value, onlineSeat.value, onlineSessionToken.value);
  battleStore.initializeSocket(socketUrl, "online");
  startTicker();
  setTimeout(() => {
    battleStore.requestRoomState();
  }, 180);
}
...
onLoad((options) => {
  ...
  if (loadOnlineSession((options ?? {}) as Record<string, string>)) {
    connectOnlineRoomBattle();
  }
...
const displayName = computed(() => {
  if (onlineMode.value) {
    const onlineName = onlineSeat.value === "host" ? onlineRoomState.value?.host.display_name : onlineRoomState.value?.guest?.display_name;
    if (onlineName?.trim()) return onlineName.trim();
  }
  return profile.nickname.trim() || "我方";
});
const avatarFallback = computed(() => displayName.value.slice(0, 1).toUpperCase());
const opponentDisplayName = computed(() => {
  if (onlineMode.value) {
    const otherName = onlineSeat.value === "host" ? onlineRoomState.value?.guest?.display_name : onlineRoomState.value?.host.display_name;
    if (otherName?.trim()) return otherName.trim();
    return "等待加入";
  }
  return "AI";
});
const opponentAvatarFallback = computed(() => opponentDisplayName.value.slice(0, 1).toUpperCase());
...
const connectionActionLabel = computed(() => {
  if (onlineMode.value) {
    if (socketStatus.value === "connecting" || socketStatus.value === "reconnecting") return "连接中";
    return socketStatus.value === "open" ? "同步" : "重连";
  }
  if (socketStatus.value === "connecting" || socketStatus.value === "reconnecting") return "连接中";
  return socketStatus.value === "open" ? "重连" : "开局";
});
const startModalTitle = computed(() => (onlineMode.value ? "联机对战" : "人机对战"));
const startModalDesc = computed(() => {
  if (!onlineMode.value) return "目前人机版本Beta1.1";
  if (!onlineRoomState.value) return "正在连接房间";
  if (!onlineRoomState.value.guest) return "正在等待玩家加入";
  if (onlineRoomState.value.status === "playing") return "双方已准备，对局正在启动";
  return `玩家 ${opponentDisplayName.value} 已经加入`;
});
const onlineGuestLine = computed(() => {
  if (!onlineMode.value) return "";
  if (!onlineRoomState.value?.guest) return "正在等待玩家加入";
  return `玩家 ${onlineRoomState.value.guest.display_name} 已加入`;
});
const startPrimaryLabel = computed(() => {
  if (!onlineMode.value) return "开始对局";
  if (!onlineRoomState.value?.guest) return "等待加入";
  if (gameStarted.value) return "已开始";
  if (myReady.value) return "已准备";
  return "准备";
});
const startPrimaryDisabled = computed(() => {
  if (!onlineMode.value) return false;
  if (!onlineRoomState.value?.guest) return true;
  if (gameStarted.value) return true;
  if (myReady.value) return true;
  return socketStatus.value !== "open";
});
const startSecondaryLabel = computed(() => (onlineMode.value ? "退出房间" : "返回"));
...
function handleStartPrimaryAction() {
  if (onlineMode.value) {
    if (startPrimaryDisabled.value) return;
    battleStore.setRoomReady(true);
    return;
  }
  handleGameStart();
}
...
function handleGameStart() {
  ...
  scheduleArenaMeasure();
  nextTick(() => {
    battleStore.initializeSocket(SOCKET_URL, "single");
    startTicker();
...
async function handleGameExit() {
  if (onlineMode.value && onlineRoomCode.value) {
    try {
      await leaveOnlineRoom(onlineRoomCode.value, {
        seat: onlineSeat.value,
      });
    } catch {
      // Ignore leave errors on exit; room cleanup is handled server-side.
    }
...
function handleCopyRoomCode() {
  if (!onlineRoomCode.value) return;
  uni.setClipboardData({
    data: onlineRoomCode.value,
    success: () => {
      uni.showToast({
        title: "房号已复制",
        icon: "none",
...
const roomClosedMessage = computed(() => {
  if (!onlineMode.value) return "房间已关闭";
  const seatValue = roomClosedSeat.value;
  switch (roomClosedReason.value) {
    case "player_disconnected":
      return seatValue === onlineSeat.value ? "你已离线，房间已关闭" : "对手已离线，房间已关闭";
    case "player_left":
      return seatValue === onlineSeat.value ? "你已退出，房间已关闭" : "对手已离开，房间已关闭";
    case "match_finished":
      return "对局结束，房间已关闭";
    default:
      return "房间已关闭";
...
watch(
  ...
  (reason) => {
    if (!onlineMode.value || !reason) return;
    clearSavedOnlineSession();
    battleStore.disconnect();
    battleStore.resetBattleState();
    uni.showToast({
      title: roomClosedMessage.value,
      icon: "none",
    });
    setTimeout(() => {
      uni.reLaunch({
...
watch(
  () => battleFinishedPayload.value,
  (payload) => {
    if (!onlineMode.value || !payload) return;
    uni.showToast({
      title: "本场对战已结束",
      icon: "none",
    });
...
.start-modal__player-row,
.start-modal__room-line {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12rpx;
...
.start-modal__player-label {
  color: #dbc89d;
  font-size: 18rpx;
  font-weight: 800;
}

.start-modal__player-name,
.start-modal__room-code {
  color: #f6edd9;
  font-size: 20rpx;
  line-height: 1.4;
}

.start-modal__state-pill {
  flex: none;
  min-width: 86rpx;
  min-height: 42rpx;
  padding: 0 14rpx;
...
.start-modal__state-pill--ready {
  color: #e7ffe7;
  background: rgba(42, 143, 80, 0.85);
}

.start-modal__state-pill--idle {
  color: #d2dde2;
  background: rgba(107, 123, 132, 0.55);
}

.start-modal__copy-btn {
  width: 48rpx;
  height: 48rpx;
  border-radius: 14rpx;
  display: inline-flex;
