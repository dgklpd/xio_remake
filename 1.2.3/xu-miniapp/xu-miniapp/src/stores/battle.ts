...
export interface OnlineRoomSeatState {
  seat: "host" | "guest";
  user_id: string | null;
  display_name: string;
  avatar_url: string;
...
export interface OnlineRoomState {
  ...
  updated_at: number;
  expires_at: number;
  host: OnlineRoomSeatState;
  guest: OnlineRoomSeatState | null;
  can_start: boolean;
}
...
interface BattleStoreState {
  ...
  roomClosedSeat: string;
  battleFinishedPayload: Record<string, unknown> | null;
}
...
function isOnlineRoomSeatState(value: unknown): value is OnlineRoomSeatState {
  return (
    isRecord(value) &&
...
function isOnlineRoomState(value: unknown): value is OnlineRoomState {
  return (
    isRecord(value) &&
    typeof value.room_code === "string" &&
    typeof value.room_name === "string" &&
    typeof value.status === "string" &&
    isOnlineRoomSeatState(value.host) &&
    (value.guest === null || isOnlineRoomSeatState(value.guest)) &&
...
export const useBattleStore = defineStore("battle", {
  ...
  actions: {
    initializeSocket(url: string, mode: "single" | "online" = "single") {
      this.mode = mode;
      this.socketUrl = url;
    ...
    resetBattleState() {
      ...
      this.roomClosedReason = "";
      this.roomClosedSeat = "";
      this.battleFinishedPayload = null;
    },
    ...
    handleServerMessage(message: SocketIncomingMessage) {
      ...
      if (message.action === "room_state" && isOnlineRoomState(message.state)) {
        this.onlineRoomState = message.state;
        this.pendingAction = null;
        return;
      }

      if (message.action === "battle_finished" && isRecord(message.payload)) {
        this.battleFinishedPayload = message.payload;
      }

      if (message.action === "room_closed") {
        if (isRecord(message.payload)) {
          this.roomClosedReason =
            typeof message.payload.reason === "string" ? message.payload.reason : "closed";
          this.roomClosedSeat = typeof message.payload.seat === "string" ? message.payload.seat : "";
        } else {
          this.roomClosedReason = "closed";
          this.roomClosedSeat = "";
        }
      }
    ...
    setRoomReady(ready: boolean) {
      return this.sendPlayerAction("ready_room", {
        ready,
      });
    },

    requestRoomState() {
      return this.sendPlayerAction("get_room_state");
    },
  },
});
